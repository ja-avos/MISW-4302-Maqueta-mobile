...
@Preview
@Composable
fun AlarmsListScreen(navController: NavController = rememberNavController()) {

    val alarms = listOf(
        "Medicamento Tos", "Comida", "Paseo diario"
    )
    ...
    Column(
        ...
        ScreenTitle(title = "Alarmas", onBack = { navController.popBackStack() })
        ...
        Column(
            ...
            alarms.forEach { alarm ->
                AlarmListItem(
                    title = alarm,
                    schedule = "L, V. 10:30 AM",
                    onClick = { navController.navigate("alarm-detail") }
                )
            }
        }
        Spacer(modifier = Modifier.height(32.dp))
        PrincipalBtn(text = "Agregar alarma",
            modifier = Modifier.padding(horizontal = 40.dp),
            onClick = { navController.navigate("alarm-create") })
    }
}